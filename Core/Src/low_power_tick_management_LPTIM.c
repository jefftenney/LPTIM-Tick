...
void vPortSuppressTicksAndSleep( TickType_t xExpectedIdleTime )
{
  ...
  ulReloadValue = (ulReloadValueForOneSec * (xExpectedIdleTime - 1UL))/configTICK_RATE_HZ;
    ...
    HAL_LPTIM_TimeOut_Start_IT(&LptimHandle, 0xFFFF, ulReloadValue);
		...
		configPRE_SLEEP_PROCESSING(0); // Jeff - was "STOP", no parameter
		...
		__DSB(); // Jeff - was HAL_PWREx_EnterSTOP1Mode(PWR_STOPENTRY_WFI);
		__WFI();
		__ISB();
		...
		configPOST_SLEEP_PROCESSING(0); // Jeff - was "STOP", no parameter
